...
void refresh_slot()
{
  for (int i = 0; i < MAXPROCS; i++) {
    ...
    Printf("utilization of proc %d is %f", proctab[i].pid, proctab[i].utilization)
...
int get_unfair_pid()
{
  ...
  for(int i = 0; i < MAXPROCS; i++){
    ...
    if (proctab[i].valid == 0){
      ...
      double utilization = (double)proctab[i].ran_slot / proctab[i].alive_slot;
      proctab[i].utilization = utilization;
